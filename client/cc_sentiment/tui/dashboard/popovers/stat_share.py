...
from cc_sentiment.tui.dashboard.view import CtaState
...
class StatShareScreen(Dialog[None]):
    ...
    StatShareScreen > #dialog-box > .actions {
        width: 100%; height: auto; align-horizontal: center; margin: 1 0 0 0;
    }
    StatShareScreen > #dialog-box > .actions > Button { margin: 0 1 0 0; }
    """
    ...
    def on_tweet_button(self) -> None:
        if self.share_id is None:
            return
        self.app.open_url(Uploader.tweet_url(self.share_id, self.stat.tweet_text))
        self.dismiss(None)
