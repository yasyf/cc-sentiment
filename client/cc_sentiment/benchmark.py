...
class BenchmarkRunner:
    ...
    def collect_buckets(max_transcripts: int) -> list[ConversationBucket]:
        ...
        with click.progressbar(transcripts, label="Parsing transcripts") as bar:
            for path in bar:
                if messages := TranscriptParser.parse_file(path):
                    all_buckets.extend(ConversationBucketer.bucket_messages(messages))
        return all_buckets
    ...
    @staticmethod
    def create_engine(engine_name: str, model_repo: str | None = None) -> InferenceEngine:
        match engine_name:
            case "mlx":
            ...
            case "omlx":
                ...
                return OMLXEngine(model_repo=model_repo)
            case _:
    ...
    def run_engine(
        ...
        engine = cls.create_engine(engine_name, model_repo)
        load_time = time.monotonic() - load_start

        click.echo("  Warmup run...")
        engine.score_buckets(buckets[:min(3, len(buckets))])
        ...
        for run_idx in range(runs):
            ...
            last_scores = engine.score_buckets(buckets)
            total_wall += time.monotonic() - wall_start

        peak_memory_gb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / (1024 ** 3)
...
def run_benchmark(
    ...
    for engine_name in engines:
        ...
        result = BenchmarkRunner.run_engine(engine_name, buckets, runs, model_repo)
