    ...
    def bucket_messages(
        ...
        for session_id, session_msgs in by_session.items():
            ...
            for idx, bucket_msgs in sorted(grouped.items()):
                if not any(
                    isinstance(m, UserMessage) and len(m.content) >= MIN_USER_CHARS
...
class PythonBackend:
    ...
    def bucket_keys_for(self, path: Path) -> list[BucketKey]:
        by_session: dict[SessionId, list[tuple[datetime, bool, bool]]] = defaultdict(list)
        for line in path.read_bytes().split(b"\n"):
            ...
            match data.get("type"):
                ...
                case "user":
                    ...
                    if not stripped:
                        continue
                    is_user = True
                    is_substantive = len(stripped) >= MIN_USER_CHARS
                case "assistant" if data["message"]["model"] == "<synthetic>":
                    continue
                case "assistant":
                    blocks = data["message"]["content"]
            ...
            by_session[SessionId(data["sessionId"])].append(
                (datetime.fromisoformat(data["timestamp"]), is_user, is_substantive)
            )

        keys: list[BucketKey] = []
        for session_id, entries in by_session.items():
            if sum(1 for _, is_user, _ in entries if is_user) < MIN_USER_TURNS_PER_SESSION:
                continue
            ...
            for ts, is_user, is_substantive in entries:
                ...
                grouped[idx].append((is_user, is_substantive))
            keys.extend(
                BucketKey(session_id=session_id, bucket_index=BucketIndex(idx))
                for idx, flags in grouped.items()
                if any(is_user and is_sub for is_user, is_sub in flags)
                and any(not is_user for is_user, _ in flags)
            )
        return keys
