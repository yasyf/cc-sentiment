...
async def test_set_total_renders_eta_when_hardware_estimates(tmp_path: Path, auth_ok):
    ...
    with patch("cc_sentiment.tui.dashboard.lifecycle.EngineFactory.resolve", return_value="mlx"), \
        ...
        async with app.run_test() as pilot:
            ...
            app.dashboard._begin_scoring(1200, "mlx", 0)
            label_text = str(app.dashboard.query_one("#progress-label", Label).render())
            assert "00:02:00" in label_text
            assert app.dashboard.status_text == ""
...
async def test_set_total_omits_eta_when_hardware_unknown(tmp_path: Path, auth_ok):
    ...
    with patch("cc_sentiment.tui.dashboard.lifecycle.EngineFactory.resolve", return_value="mlx"), \
        ...
        async with app.run_test() as pilot:
            await pilot.pause(delay=0.3)
            app.dashboard._begin_scoring(500, "mlx", 0)
            label_text = str(app.dashboard.query_one("#progress-label", Label).render())
...
async def test_add_buckets_updates_progress(tmp_path: Path, auth_ok):
    ...
    with patch("cc_sentiment.tui.dashboard.lifecycle.EngineFactory.resolve", return_value="mlx"), \
        ...
        async with app.run_test() as pilot:
            await pilot.pause(delay=0.3)
            app.dashboard._begin_scoring(100, "mlx", 0)
            app.dashboard._add_buckets(5)
            app.dashboard._add_buckets(3)
            assert app.dashboard.scored == 8
