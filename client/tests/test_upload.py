...
class TestPendingRecords:
    async def test_returns_records_from_non_uploaded_sessions(self, repo: Repository) -> None:
        record = make_record()
        await repo.save_records("/p.jsonl", 1.0, [record])

        result = await repo.pending_records()
        assert len(result) == 1
        assert result[0] == record

    async def test_skips_uploaded_sessions(self, repo: Repository) -> None:
        uploaded = make_record(session_id="s1")
        pending = make_record(session_id="session-2")
        await repo.save_records("/p.jsonl", 1.0, [uploaded, pending])
        await repo.mark_sessions_uploaded({SessionId("s1")})

        result = await repo.pending_records()
        assert len(result) == 1
        assert result[0].conversation_id == SessionId("session-2")

    async def test_returns_empty_when_all_uploaded(self, repo: Repository) -> None:
        record = make_record()
        await repo.save_records("/p.jsonl", 1.0, [record])
        await repo.mark_sessions_uploaded({record.conversation_id})

        assert await repo.pending_records() == []


class TestUpload:
    async def test_raises_when_config_is_none(self, repo: Repository) -> None:
    ...
    async def test_marks_sessions_uploaded_after_success(self, repo: Repository) -> None:
        record = make_record()
        await repo.save_records("/p.jsonl", 1.0, [record])
