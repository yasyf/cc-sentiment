...
fn classify_envelope(env: Envelope<'_>) -> Option<(String, i64, bool, bool)> {
    if env.entrypoint.as_deref() == Some("sdk-cli") {
        return None;
    }
    let (is_user, is_substantive) = match env.kind.as_deref()? {
        "user" => {
            if env.is_sidechain {
                return None;
            }
            ...
            let trimmed_len = match content_val {
                Content::Text(s) => {
                    if JUNK_RE.is_match(s) {
                        return None;
                    }
                    s.trim().chars().count()
                }
            ...
            (true, trimmed_len >= MIN_USER_CHARS)
        }
        "assistant" => {
            let msg = env.message.as_ref()?;
    ...
    Some((session_id, ts_secs, is_user, is_substantive))
}
...
fn handle_scan_line(
    ...
    let Some((session_id, ts_secs, is_user, is_substantive)) = classify_envelope(env) else {
        return;
    };
    by_session
        .entry(session_id)
