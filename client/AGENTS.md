...
- **One screen per file under `tui/screens/`.** `BootingScreen`, `PlatformErrorScreen`, `StatShareScreen`, `CostReviewScreen`, `SetupScreen`. The screen class is the only top-level export. Its private helpers stay underscore-prefixed methods on the screen class itself.
